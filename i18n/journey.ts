export const journeyContent = {
  es: {
    title: "Mi trayectoria personal",
    tabs: {
      education: "Educación",
      work: "Experiencia",
      courses: "Cursos",
      projects: "Proyectos",
    },

    education: [
      {
        title: "Posgrado en IA y Machine Learning",
        subtitle: "Cambrian College, Sudbury, Canadá",
        date: "2025 - 2026",
      },
      {
        title: "Máster en Ciberseguridad",
        subtitle: "Universidad Isabel I, Madrid, España",
        date: "2023 - 2024",
      },
      {
        title: "Ingeniería de Sistemas",
        subtitle: "Universitaria de Colombia, Bogotá, Colombia",
        date: "2020 - 2023",
      },
      {
        title: "Técnico en Análisis y Desarrollo de Sistemas de Información",
        subtitle: "SENA, Bogotá, Colombia",
        date: "2020 - 2022",
      },
      {
        title: "Diplomado en Desarrollo de Aplicaciones Móviles",
        subtitle: "UTP & Misión TIC, Bogotá, Colombia",
        date: "2021",
      },
    ],

    work: [
      {
        title: "Desarrollador Full Stack",
        subtitle: "Symplifica, Bogotá, Colombia",
        date: "10/2023 - Presente",
      },
      {
        title: "Full Stack Developer Freelance",
        subtitle: "BotsLovers, Madrid, España",
        date: "02/2025 - 04/2025",
      },
      {
        title: "Analista Survey Scripter",
        subtitle: "Ipsos, Bogotá, Colombia",
        date: "10/2022 - 02/2024",
      },
      {
        title: "Ethical Hacker Freelance",
        subtitle: "Enterkey LLC, Miami, USA",
        date: "01/2023 - 06/2023",
      },
      {
        title: "Full Stack Developer & Scrum Master (Practicante)",
        subtitle: "Universitaria de Colombia, Bogotá, Colombia",
        date: "03/2022 - 03/2023",
      },
      {
        title: "Web Master",
        subtitle: "Universitaria de Colombia, Bogotá, Colombia",
        date: "12/2021 - 08/2022",
      },
    ],

    courses: [
      "Introducción a la Inteligencia Artificial – Platzi",
      "Introducción a Machine Learning – Platzi",
      "Herramientas de IA para Developers – Platzi",
      "Ethical Hacking – Universidad El Bosque",
      "Ethical Hacking – Platzi",
      "Photoshop – SENA",
      "Electrónica – SENA",
      "Desarrollo Web I & II – Google",
      "Ciberseguridad para PYMES – Google",
      "Aprendiz Digital – SENA",
      "Arquitectura de Computadores – SENA",
      "Aseguramiento de la Calidad de Software – SENA",
      "AutoCAD – SENA",
      "Inglés Nivel 9 – SENA",
      "Python – Udemy",
      "Certificación SCRUM: Fundamentos",
    ],

    projects: [
      {
        title: "RescuePets",
        description: "Proyecto de Misión TIC para facilitar la adopción de mascotas.",
        image: "https://santic.netlify.app/assets/img/Rescuepets.png",
        url: "https://rescuepets.netlify.app/",
      },
      {
        title: "Proyecto de Grado",
        description: "Proyecto final de ingeniería de sistemas desarrollado en equipo.",
        image: "https://santic.netlify.app/assets/img/Asistencia.png",
        url: "https://santic.netlify.app/proyectogrado",
      },
      {
        title: "Investigación de Virus Informáticos",
        description: "Investigación sobre el origen y propagación de virus.",
        image: "https://santic.netlify.app/assets/img/computador.jpg",
        url: "https://santic.netlify.app/virus_investigation",
      },
      {
        title: "Security Bikes",
        description: "Proyecto de aprendizaje en desarrollo web y bases de datos.",
        image: "https://santic.netlify.app/assets/img/security.png",
        url: "#",
      },
      {
        title: "Programa de Citas en C#",
        description: "Proyecto final del programa ADSI.",
        image: "https://santic.netlify.app/assets/img/citas.png",
        url: "https://www.youtube.com/watch?v=4S0SahKvL7Q",
      },
    ],
  },

  en: {
    title: "My personal journey",
    tabs: {
      education: "Education",
      work: "Work",
      courses: "Courses",
      projects: "Projects",
    },

    education: [
      {
        title: "Postgraduate Degree in AI and Machine Learning",
        subtitle: "Cambrian College, Sudbury, Canada",
        date: "2025 - 2026",
      },
      {
        title: "Master in Cybersecurity",
        subtitle: "Isabel I University, Madrid, Spain",
        date: "2023 - 2024",
      },
      {
        title: "Degree in Systems Engineering",
        subtitle: "Universitaria de Colombia University, Bogotá, Colombia",
        date: "2020 - 2023",
      },
      {
        title: "Technician in Information Systems Development",
        subtitle: "SENA, Bogotá, Colombia",
        date: "2020 - 2022",
      },
      {
        title: "Mobile Application Development Diploma",
        subtitle: "UTP & Mision TIC, Bogotá, Colombia",
        date: "2021",
      },
    ],

    work: [
      {
        title: "Full Stack Developer",
        subtitle: "Symplifica, Bogotá, Colombia",
        date: "10/2023 - Present",
      },
      {
        title: "Freelance Full Stack Developer",
        subtitle: "BotsLovers, Madrid, Spain",
        date: "02/2025 - 04/2025",
      },
      {
        title: "Survey Scripter Analyst",
        subtitle: "Ipsos, Bogotá, Colombia",
        date: "10/2022 - 02/2024",
      },
      {
        title: "Freelance Ethical Hacker",
        subtitle: "Enterkey LLC, Miami, USA",
        date: "01/2023 - 06/2023",
      },
      {
        title: "Full Stack Developer & Scrum Master Intern",
        subtitle: "Universitaria de Colombia University, Bogotá, Colombia",
        date: "03/2022 - 03/2023",
      },
      {
        title: "Web Master",
        subtitle: "Universitaria de Colombia University, Bogotá, Colombia",
        date: "12/2021 - 08/2022",
      },
    ],

    courses: [
      "Introduction to Artificial Intelligence – Platzi",
      "Introduction to Machine Learning – Platzi",
      "AI Tools for Developers – Platzi",
      "Ethical Hacking – El Bosque University",
      "Ethical Hacking – Platzi",
      "Photoshop – SENA",
      "Electronics – SENA",
      "Web Development I & II – Google",
      "Cybersecurity for SMEs – Google",
      "Digital Apprentice – SENA",
      "Computer Architecture – SENA",
      "Software Quality Assurance – SENA",
      "AutoCAD – SENA",
      "English Level 9 – SENA",
      "Python – Udemy",
      "SCRUM Certification: Fundamentals",
    ],

    projects: [
      {
        title: "RescuePets",
        description: "A Mision TIC project designed to facilitate pet adoption.",
        image: "https://santic.netlify.app/assets/img/Rescuepets.png",
        url: "https://rescuepets.netlify.app/",
      },
      {
        title: "Graduation Project",
        description: "Final systems engineering project developed in a team.",
        image: "https://santic.netlify.app/assets/img/Asistencia.png",
        url: "https://santic.netlify.app/proyectogrado",
      },
      {
        title: "Cyber Virus Research",
        description: "Research about the origin and propagation of computer viruses.",
        image: "https://santic.netlify.app/assets/img/computador.jpg",
        url: "https://santic.netlify.app/virus_investigation",
      },
      {
        title: "Security Bikes",
        description: "Project to learn web development and databases.",
        image: "https://santic.netlify.app/assets/img/security.png",
        url: "#",
      },
      {
        title: "C# Appointment Program",
        description: "Final project for the ADSI program.",
        image: "https://santic.netlify.app/assets/img/citas.png",
        url: "https://www.youtube.com/watch?v=4S0SahKvL7Q",
      },
    ],
  },
} as const;

export type Lang = keyof typeof journeyContent;
export type TabKey = keyof typeof journeyContent.en.tabs;
